...
echo "Starting FINAL Sectioned Fair Sync...\n";
...
function processFair($slugPart, $type, $tr, $months) {
    $url = '/fuarlar/' . $slugPart;
    ...
    try {
        ...
        // Use the slug part from URL to avoid collisions
        $fair = Fair::where('slug', $slugPart)->first() ?: new Fair(['slug' => $slugPart]);
        $fair->name = $name;
        $fair->type = $type;
        ...
        if (isset($descMatch[1])) {
            $desc = $descMatch[1];
            // Robustly strip the redundant metadata list and everything following it if it's unclosed
            $desc = preg_replace('/<div class="fuar-detail-card-content-responsive">.*/s', '', $desc);
            $desc = trim(preg_replace('/<h2>.*?<\/h2>/s', '', $desc));
